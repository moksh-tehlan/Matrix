...
public class DocumentService {
    ...
    public List<KnowledgeSourceEntity> uploadDocumentAndParseIt(List<MultipartFile> files) {
        ...
        try {
            ...
            for (MultipartFile file : files) {
                CompletableFuture<Void> future = CompletableFuture.runAsync(() -> {
                    ...
                    try {
                        ...
                        KnowledgeSourceEntity knowledgeSource = KnowledgeSourceEntity.builder()
                                .path(key)
                        ...
                        KnowledgeSourceEntity savedKnowledgeSource = knowledgeSourceService.saveKnowledgeSource(knowledgeSource);
                        s3Service.uploadFile(bucketName, key, fileByteArray);

                        long endTime = System.currentTimeMillis();
                        log.info("S3 upload for {} took {} ms", file.getOriginalFilename(), (endTime - startTime));

                        // Start async processing
                        parserService.processDocumentAsync(bucketName, key, savedKnowledgeSource.getId());
                        sources.add(savedKnowledgeSource);
                    } catch (Exception e) {
                        log.error("Error processing file {}", file.getOriginalFilename(), e);
                        throw new DocumentProcessingException("Failed to upload/process file: " + file.getOriginalFilename(), e);
                    }
                }, executorService);
                futures.add(future);
            }
            ...
            throw new DocumentProcessingException("Failed to upload and parse documents: " + e.getMessage(), e);
        }
    }
