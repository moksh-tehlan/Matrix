...
public class ChatMemoryService implements ChatMemory {
  ...
  public void add(String conversationId, List<Message> messages) {
    ...
    log.debug(
        "Adding {} messages to chat memory for conversation: {}", messages.size(), conversationId);
  ...
  public List<Message> get(String conversationId, int lastN) {
    long startTime = System.currentTimeMillis();
    log.debug(
        "Retrieving messages from chat memory for conversation: {}, lastN: {}",
        conversationId,
        lastN);
    ...
    try {
      ...
      long endTime = System.currentTimeMillis();
      log.debug(
          "Retrieved {} messages from chat memory in {} ms",
          messages.size(),
          (endTime - startTime));
