...
public class SlackMessageService {
    ...
    public boolean sendMessage(String channelId, String workspaceId, String message) {
        try {
            ...
            // Send message
            ChatPostMessageResponse response = sendSlackMessage(channelId, message, workspace.getBotToken());

            if (!response.isOk()) {
                log.error("Failed to send message to Slack: {}", response.getError());
                throw new SlackIntegrationException("Failed to send message to Slack: " + response.getError());
            }
     ...
     */
    private ChatPostMessageResponse sendSlackMessage(String channelId, String message, String botToken)
