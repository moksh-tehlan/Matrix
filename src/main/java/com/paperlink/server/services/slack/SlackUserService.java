...
public class SlackUserService {
  ...
  public SlackUserEntity getOrCreateSlackUser(String slackUserId, String workspaceId) {
    try {
      ...
      Optional<SlackUserEntity> existingUser =
          slackUserRepository.findBySlackUserIdAndWorkspaceId(slackUserId, workspaceId);
      ...
      if (!response.isOk()) {
        ...
        throw new SlackIntegrationException(
            "Failed to get user info from Slack: " + response.getError());
      }

      // Create new Slack user entity
      SlackUserEntity newUser =
          SlackUserEntity.builder()
   ...
   */
  private UsersInfoResponse getUserInfoFromSlack(String slackUserId, String botToken)
      throws IOException, SlackApiException {
    MethodsClient client = slack.methods(botToken);
    UsersInfoRequest request = UsersInfoRequest.builder().user(slackUserId).build();
